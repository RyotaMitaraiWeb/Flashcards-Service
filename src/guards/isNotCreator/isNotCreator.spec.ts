...
import { IsNotCreatorGuard } from './isNotCreator';

describe('isNotCreatorGuard', () => {
  ...
  it('Lets in requests from non-creator', async () => {
  ...
  it('Blocks requests from the creator', async () => {
    jest.spyOn(deckRepository, 'findOne').mockImplementation(async () => {
      const deck = new Deck();
